...
/**
 * Tournament_Service_Cup
 *
@author Andrzej Wilczyński <and.wilczynski@gmail.com>
 */
class Tournament_Service_Cup extends MF_Service_ServiceAbstract {
    
    protected $cupTable;
    
    public function init() {
        $this->cupTable = Doctrine_Core::getTable('Tournament_Model_Doctrine_Cup');
        parent::init();
    }
    ...
    public function getGoals($match_id,$team_id,$hydrationMode = Doctrine_Core::HYDRATE_RECORD)
      {
           ...
           return $q->execute(array(),$hydrationMode);
              ...
              public function getCups($tournament_id,$hydrationMode = Doctrine_Core::HYDRATE_RECORD)
              {
                  $q = $this->cupTable->createQuery('s');
                  $q->leftJoin('s.Player p');
                  $q->leftJoin('p.Team t');
