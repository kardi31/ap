 ...
 */
class Tournament_IndexController extends MF_Controller_Action {
    ...
    public function timetableAction()
    {
        ...
        if(!$tournament = $tournamentService->getTournament($this->getRequest()->getParam('tournament'),'slug')){
            throw new Zend_Exception('Tournament not found');
        }
        $data = $matchService->getTimetableDate($tournament->id);
        $timetable = $matchService->getTimetable($tournament->id);
    ...
    public function showResultAction()
    {
        ...
        $results = $matchService->getResults($tournament->id);
    ...
    public function showTableAction()
    {
        ...
        $groups = $teamService->getGroups($tournament->id,'id');
        $grupy = array();
        foreach($groups as $group):
            $grupy['Grupa '.$group['group_id']] = $matchService->getTable($tournament->id,$group['group_id']);
        endforeach;
        $this->view->assign('grupy',$grupy);
         $this->view->assign('tournament',$tournament);
         $this->_helper->actionStack('layout','index','default');
    }
    ...
    public function showShootersAction()
    {
        ...
        $results = $shooterService->getShooters($tournament->id);
    ...
    public function showBookingAction()
    {
        ...
        $yellowCards = $bookingService->getBooking($tournament->id,1);
        $redCards = $bookingService->getBooking($tournament->id,2);
    ...
    public function showSquadAction()
    {
     ...
     public function showCupAction() {
        ...
        $cupService = $this->_service->getService('Tournament_Service_Cup');
